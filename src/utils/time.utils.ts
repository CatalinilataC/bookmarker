...
export const isToday = (currentMoment: number, timestamp: number) => currentMoment - timestamp < DAY_IN_MS; // should check if it's from future

export const isYesterday = (currentMoment: number, timestamp: number) => 
        !isToday(currentMoment, timestamp) && currentMoment - timestamp <  2* DAY_IN_MS;

export const isOlderThanYesterday = (currentMoment: number, timestamp: number) => 
