  ...
  mappedData$ = this.bookmarks$.pipe(
    map((data) => {
      ...
      return {
        ...
        yesterdayData: data.filter((v) =>
          isYesterday(currentMoment, v.timestamp)
        ),
        olderData: data.filter((v) =>
          isOlderThanYesterday(currentMoment, v.timestamp)
        ),
      };
    })
  );
